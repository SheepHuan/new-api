...
func CountUserConsumeLogsSince(startTimestamp int64) (map[int]int64, error) {
	if LOG_DB == nil {
		return map[int]int64{}, nil
	}
	var rows []UserRequestCount
	err := LOG_DB.Model(&Log{}).
		Select("user_id, COUNT(*) AS request_count").
		Where("type = ? AND created_at >= ? AND user_id > 0", LogTypeConsume, startTimestamp).
		Group("user_id").
		Scan(&rows).Error
	if err != nil {
		return nil, err
	...
	for _, row := range rows {
		...
		counts[row.UserId] = row.RequestCount
	}
	return counts, nil
}
